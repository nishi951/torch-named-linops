...
from torchlinops.mri._grid.third_party._igrog.grogify import (
    grogify,
    training_params,
    gridding_params,
)
...
def test_gridded_nufft_2d(spiral2d_data):
    ...
    img_cal = ConjugateGradient(
        A.N + cg_lambda * Identity(A.ishape), CGHparams(num_iter=cg_iters)
    )(AHb, AHb)
    ...
    ksp_grd, trj_grd = grogify(
        data.ksp.to(device), data.trj.to(device), img_cal.to(device), tparams, gparams
    )
    D = DCF(trj_grd, data.img.shape, ("C", "R", "K"), show_pbar=False)
    F = NUFFT(
        trj_grd,
        im_size,
        in_batch_shape=("C",),
    ...
    D = DCF(data.trj.to(device), data.img.shape, ("C", "R", "K"), show_pbar=False)
    F = NUFFT(
        data.trj,
        im_size,
        in_batch_shape=("C",),
