...
__all__ = [
    "time_segment_helper" "CalibRegion",
    "CalibHparams",
]
...
class CalibRegion:
    def __init__(
        ...
        segment_length: Optional[
            int
        ] = None,  # Length of time segment, in number of samples
        ...
        if b0_map is not None:
            ...
            ), "Must provide segment_length when b0 calibration is requested."
            self.segment_length = segment_length
            im_size = b0_map.shape
            assert len(self.cal_size) == len(
                im_size
            ), "Calibration region and B0 map have different dimensionality"
            ...
            self.temporal_ksp_cal_pre_KB = self._FT_pre_KB(
                img_cal=temporal_img_cal, spatial_oshape=ksp_cal_rs.shape[1:]
            )
            self.temporal_orig_shape = temporal_img_cal.shape
            self.temporal_ksp_cal_pre_KB = sp.to_device(
                self.temporal_ksp_cal_pre_KB, self.device
            )

        # Kinda silly but its ok
        with self.device:
            self.img_cal = sp.to_device(
                sp.ifft(self.ksp_cal, axes=range(-self.dim, 0)), self.device
            )
        self.orig_shape = self.img_cal.shape
        self.ksp_cal_pre_KB = self._FT_pre_KB(
            img_cal=self.img_cal, spatial_oshape=self.ksp_cal.shape[1:]
        )
    ...
    def __call__(
        ...
        if dts is not None:
            assert (
                loc.shape[0] == dts.shape[0]
            ), "times and locations must have same batch size"
            assert (
                dts.min() >= -self.dt_max and dts.max() <= self.dt_max
            ...
            with self.device:
                ...
                for idx in lower_idx_set:
                    ...
                    # spatial KB interp
                    ksp_cal_idx = sp.to_device(
                        self.temporal_ksp_cal_pre_KB[idx], self.device
                    )
                    nufft_lower = self._kb_interp(
                        ksp_cal_pre_KB=ksp_cal_idx,
                        orig_shape=self.temporal_orig_shape,
                        loc=loc_interp,
                    )
                    ksp_cal_idx = sp.to_device(
                        self.temporal_ksp_cal_pre_KB[idx + 1], self.device
                    )
                    nufft_upper = self._kb_interp(
                        ksp_cal_pre_KB=ksp_cal_idx,
                        orig_shape=self.temporal_orig_shape,
                        loc=loc_interp,
                    )
                    ...
                    nufft = (
                        alpha * nufft_lower + (1 - alpha) * nufft_upper
                    )  # [nc, npoints]
                    ksp[mask, :] = nufft.T
                return ksp
        else:
    ...
    def _FT_pre_KB(
        ...
        with self.device:
            ...
            ksp_cal_pre_KB = self.device.xp.zeros(
                (output_rs.shape[0], *spatial_oshape), dtype=np.complex64
            )
            for i, j in batch_iterator(output_rs.shape[0], fft_batch_size):
                full_ksp = sp.fft(
                    sp.to_device(output_rs[i:j], self.device),
                    axes=range(-d, 0),
                    norm=None,
                )
                ksp_cal_pre_KB[i:j] = sp.resize(
