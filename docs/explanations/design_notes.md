...
## SyncContext: hierarchical execution context

When a linop is called, it receives an optional `SyncContext` that carries execution metadata through the call stack. This is essential for multi-GPU synchronization.

### How SyncContext works

`SyncContext` is a dataclass created fresh on each `forward()` call:

```python
@dataclass
class SyncContext:
    linop: type[NamedLinop]       # Class of linop that owns this context
    input_device: torch.device     # Device of input to this linop
    parent: Optional[SyncContext]  # Calling context
    start_event: torch.cuda.Event | None  # Synchronization event
```

The context is built hierarchically:

1. **Container linops** (those with `is_container = True`, like `Add`, `Concat`, `Stack`, `Chain`) record a CUDA event on the input device's current stream when their input is CUDA. This event marks "everything before this point is done."

2. **Child linops** check if their parent is a container with a `start_event`. If so, they **reuse** the parent's event rather than recording a new one. This avoids redundant synchronization — all children of a parallel container start from the same synchronization point.

3. **Non-container linops** that are children of non-containers record their own fresh event.

```python
# Simplified flow:
# Chain(A, B, C)(x)
#   -> Chain records start_event on input stream
#   -> A receives context with Chain's start_event (reused)
#   -> B, C receive context but don't need parent event (sequential)

# Add(A, B, C)(x)
#   -> Add records start_event on input stream
#   -> A, B, C each receive context with Add's start_event (all reused)
#   -> All three can start in parallel once the event completes
```

### Event propagation in nested composites

When composite linops are nested, each level creates its own `SyncContext` with a reference to its parent:
...
1. `Chain` creates `SyncContext(linop=Chain, parent=None)`, records event
2. `Add` creates `SyncContext(linop=Add, parent=Chain's context)`, reuses Chain's event (Chain is a container)
3. `A` and `B` each create their own context with `parent=Add's context`, reuse Add's event (Add is a container)
4. `C` creates `SyncContext(linop=C, parent=Chain's context)`

This hierarchical structure ensures that synchronization events flow correctly through arbitrarily deep nesting without manual intervention.

### The `context` parameter in `fn` and `adj_fn`

Subclasses that need access to the execution context can declare `context` in their `fn` signature:

```python
@staticmethod
def fn(linop, x: Tensor, /, context=None):
    # context is a SyncContext with start_event for CUDA synchronization
    return parallel_execute(
        ...
        reduce_fn=sum,
        parent=linop,
        threaded=linop.threaded,
        num_workers=linop.num_workers,
    )
```

The `forward()` method inspects the `fn` signature at runtime — if `context` is a parameter, it constructs and passes a `SyncContext`; otherwise, it calls `_run(x)` without context. This keeps the API backward-compatible for simple linops that don't need synchronization.
