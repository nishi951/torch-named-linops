...
The splitting, device transfer, and reassembly logic is handled by the library. Each operator only needs to implement `split` to describe how it decomposes along its dimensions. See [Multi-GPU Execution](multi_gpu.md) for the full details.
